...
function renderWorlds() {
  ...
  main.append(
    ...
    el("div", { style: "text-align:center;margin-top:10px" }, [
      ...
      el("button", { class: "btn ghost small", text: "♻ 처음부터 다시", onclick: () => { if (confirm("⚠️ 지금까지 연 문(진행 기록)이 모두 사라져요.\n정말 처음부터 다시 시작할까요?")) { const g = Store.getGroup(); const wasTeacher = teacherOn(); Store.reset(); if (g) Store.setGroup(g); toast("처음부터 다시!"); go({ screen: (!URL_GROUP && wasTeacher) ? "teacher" : "start" }); } } }),
    ]),
  );
}
